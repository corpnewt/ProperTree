...
class ProperTree:
    def __init__(self, plists = []):
        ...
        if str(sys.platform) == "darwin":
            ...
            file_menu.add_command(label="New", command=self.new_plist, accelerator="{}+N".format(sign))
            file_menu.add_command(label="Open", command=self.open_plist, accelerator="{}+O".format(sign))
            file_menu.add_command(label="Save", command=self.save_plist, accelerator="{}+S".format(sign))
            file_menu.add_command(label="Save As...", command=self.save_plist_as, accelerator="{}+Shift+S".format(sign))
            file_menu.add_command(label="Duplicate", command=self.duplicate_plist, accelerator="{}+D".format(sign))
            file_menu.add_command(label="Reload From Disk", command=self.reload_from_disk, accelerator="{}+L".format(sign))
            file_menu.add_separator()
            file_menu.add_command(label="OC Snapshot", command=self.oc_snapshot, accelerator="{}+R".format(sign))
            file_menu.add_command(label="OC Clean Snapshot", command=self.oc_clean_snapshot, accelerator="{}+Shift+R".format(sign))
            file_menu.add_separator()
            file_menu.add_command(label="Convert Window", command=self.show_convert, accelerator="{}+T".format(sign))
            file_menu.add_command(label="Strip Comments", command=self.strip_comments, accelerator="{}+M".format(sign))
            file_menu.add_separator()
            file_menu.add_command(label="Settings",command=self.show_settings, accelerator="{}+,".format(sign))
            file_menu.add_separator()
            file_menu.add_command(label="Toggle Find/Replace Pane",command=self.hide_show_find, accelerator="{}+F".format(sign))
            file_menu.add_command(label="Toggle Plist/Data Type Pane",command=self.hide_show_type, accelerator="{}+P".format(sign))
            file_menu.add_separator()
            file_menu.add_command(label="Quit", command=self.quit, accelerator="{}+Q".format(sign))
