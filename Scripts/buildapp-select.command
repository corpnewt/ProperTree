...
def get_os_version():
    p = subprocess.Popen(["sw_vers", "-productVersion"], shell=False,
...
def gather_python(show_all=False):
    ...
    for path in pypaths:
        ...
        path = path.strip()
        p = subprocess.Popen([path, "-V"], shell=False,
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        c = p.communicate()
        ...
        p = subprocess.Popen([path, "-c", command], shell=False,
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        c = p.communicate()
    ...
    while True:
        ...
        for i, x in enumerate(py_versions, 1):
            print("{}. {}{}{}{}".format(
                ...
                "" if min_tk == None or x[2] == None or x[2] >= min_tk else " ({}+ recommended)".format(
                    min_tk),
            ))
        print("")
        if current:
            print("C. Current ({})".format(current))
        print("Q. Quit")
        print("")
...
def main():
    ...
    if os.path.exists("ProperTree.app/Contents/MacOS/ProperTree.command"):
        ...
        try:
            ...
            pass
    min_tk = get_min_tk_version()
    py_version = py_versions[0] if len(py_versions) == 1 else select_py(
        py_versions, min_tk, pt_current)
    os.system("clear")
    ...
    if os.path.exists("ProperTree.app"):
        ...
        if os.path.exists("ProperTree.app/Contents/MacOS/Scripts/settings.json"):
            ...
            shutil.copy("ProperTree.app/Contents/MacOS/Scripts/settings.json",
    ...
    with open("ProperTree.app/Contents/MacOS/ProperTree.command", "w") as f:
        f.write("\n".join(ptcom))
    # chmod +x
    p = subprocess.Popen(["chmod", "+x", "ProperTree.app/Contents/MacOS/ProperTree.command"],
    ...
    for x in os.listdir("Scripts"):
        if x.startswith(".") or not x.lower().endswith((".py", ".plist", ".icns")):
            continue
        print(" - "+x)
        target = "ProperTree.app/Contents/Resources" if x.lower().endswith(
            ".icns") else "ProperTree.app/Contents/MacOS/Scripts"
    ...
    if temp:
        ...
        shutil.copy(os.path.join(temp, "settings.json"),
