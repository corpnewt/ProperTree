...
class EntryPopup(tk.Entry):
    ...
    def next_field(self, event):
        ...
        if self.column == "#0":
            ...
            check_type = "dictionary" if not len(
                parent) else self.master.get_check_type(parent).lower()
        ...
        if self.column == "#0":
            ...
            for child in self.parent.get_children(parent):
                ...
                if text == self.parent.item(child, "text"):
                    ...
                    return
            # Add to undo stack
            self.master.add_undo({"type": "edit", "cell": self.cell, "text": self.parent.item(
                self.cell, "text"), "values": self.parent.item(self.cell, "values")})
            ...
            # Add to undo stack
            self.master.add_undo({"type": "edit", "cell": self.cell, "text": self.parent.item(
                self.cell, "text"), "values": original})
...
"""
        ...
        self.bind("<Configure>", lambda event,
                  obj=self: self.window_resize(event, obj))
        ...
        self.type_menu.add_command(label="Dictionary", command=lambda: self.change_type(
            self.menu_code + " Dictionary"))
        self.type_menu.add_command(
            label="Array", command=lambda: self.change_type(self.menu_code + " Array"))
        self.type_menu.add_separator()
        self.type_menu.add_command(
            label="Boolean", command=lambda: self.change_type(self.menu_code + " Boolean"))
        self.type_menu.add_command(
            label="Data", command=lambda: self.change_type(self.menu_code + " Data"))
        self.type_menu.add_command(
            label="Date", command=lambda: self.change_type(self.menu_code + " Date"))
        self.type_menu.add_command(
            label="Number", command=lambda: self.change_type(self.menu_code + " Number"))
        self.type_menu.add_command(
            label="String", command=lambda: self.change_type(self.menu_code + " String"))
        ...
        self.bool_menu.add_command(
            label="True", command=lambda: self.set_bool("True"))
        self.bool_menu.add_command(
            label="False", command=lambda: self.set_bool("False"))
        ...
        self._tree = ttk.Treeview(self._tree_frame, columns=(
            "Type", "Value"), selectmode="browse", style=self.style_name)
        self._tree.heading("#0", text="Key")
        self._tree.heading("#1", text="Type")
        self._tree.heading("#2", text="Value")
        self._tree.column("Type", width=int(
            self._tree.winfo_reqwidth()/4), stretch=False)
        # self._tree.column("Drag",minwidth=40,width=40,stretch=False,anchor="center")

        # Boolean type display (YES/NO & True/False)
        self.bool_options = [["True", "False"], ["YES", "NO"]]
        self.change_yes_no_recursively(self._tree.focus())
        ...
        # Bind right click
        if str(sys.platform) == "darwin":
            # ButtonRelease-2 on mac
            self._tree.bind("<ButtonRelease-2>", self.popup)
            self._tree.bind("<Control-ButtonRelease-1>",
                            self.popup)  # Ctrl+Left Click on mac
        else:
        ...
        if not str(sys.platform) == "darwin":
            ...
            file_menu.add_command(
                label="New", command=self.controller.new_plist, accelerator="Ctrl+N")
            file_menu.add_command(
                label="Open", command=self.controller.open_plist, accelerator="Ctrl+O")
            file_menu.add_command(
                label="Save", command=self.controller.save_plist, accelerator="Ctrl+S")
            file_menu.add_command(
                label="Save As...", command=self.controller.save_plist_as, accelerator="Ctrl+Shift+S")
            file_menu.add_command(
                label="Duplicate", command=self.controller.duplicate_plist, accelerator="Ctrl+D")
            file_menu.add_command(
                label="Reload From Disk", command=self.reload_from_disk, accelerator="Ctrl+L")
            file_menu.add_separator()
            file_menu.add_command(
                label="OC Snapshot", command=self.oc_snapshot, accelerator="Ctrl+R")
            file_menu.add_command(
                label="OC Clean Snapshot", command=self.oc_clean_snapshot, accelerator="Ctrl+Shift+R")
            file_menu.add_separator()
            file_menu.add_command(label="Convert Window", command=lambda: self.controller.show_window(
                self.controller.tk), accelerator="Ctrl+T")
            file_menu.add_command(
                label="Strip Comments", command=self.strip_comments, accelerator="Ctrl+M")
            file_menu.add_command(label="Strip Disabled Entries",
                                  command=self.strip_disabled, accelerator="Ctrl+E")
            file_menu.add_separator()
            file_menu.add_command(label="Settings", command=lambda: self.controller.show_window(
                self.controller.settings_window), accelerator="Ctrl+,")
            file_menu.add_separator()
            file_menu.add_command(label="Toggle Find/Replace Pane",
                                  command=self.hide_show_find, accelerator="Ctrl+F")
            file_menu.add_command(label="Toggle Plist/Data/Int Type Pane",
                                  command=self.hide_show_type, accelerator="Ctrl+P")
            file_menu.add_separator()
            file_menu.add_command(
                label="Quit", command=self.controller.quit, accelerator="Ctrl+Q")
        ...
        self.plist_type_menu = tk.OptionMenu(
            self.display_frame, self.plist_type_string, "XML", "Binary", command=self.change_plist_type)
        self.plist_type_string.set("XML")
        self.data_type_string = tk.StringVar(self.display_frame)
        self.data_type_menu = tk.OptionMenu(
            self.display_frame, self.data_type_string, "Hex", "Base64", command=self.change_data_type)
        self.data_type_string.set("Hex")
        self.int_type_string = tk.StringVar(self.display_frame)
        self.int_type_menu = tk.OptionMenu(
            self.display_frame, self.int_type_string, "Decimal", "Hex", command=self.change_int_type)
        ...
        self.plist_type_menu.grid(
            row=1, column=2, padx=10, pady=10, sticky="we")
        self.data_type_menu.grid(
            row=1, column=4, padx=10, pady=10, sticky="we")
        ...
        self.r_text.grid(row=1, column=2, columnspan=1,
                         sticky="we", padx=10, pady=10)
        f_title = tk.StringVar(self.find_frame)
        f_title.set("Key")
        f_option = tk.OptionMenu(self.find_frame, f_title, "Key", "Boolean",
                                 "Data", "Date", "Number", "String", command=self.change_find_type)
        f_option['menu'].insert_separator(1)
        f_option.grid(row=0, column=1)
        self.fp_button = tk.Button(
            self.find_frame, text="< Prev", width=8, command=self.find_prev)
        self.fp_button.grid(row=0, column=3, sticky="e", padx=0, pady=10)
        self.fn_button = tk.Button(
            self.find_frame, text="Next >", width=8, command=self.find_next)
        ...
        self.r_all = tk.Checkbutton(
            self.find_frame, text="Replace All", variable=self.r_all_var)
        self.r_all.grid(row=1, column=5, sticky="w")
        self.f_case_var = tk.IntVar()
        self.f_case = tk.Checkbutton(
            self.find_frame, text="Case-Sensitive", variable=self.f_case_var)
    ...
    def set_font_size(self):
        self.font["size"] = self.controller.font_string.get(
        ) if self.controller.custom_font.get() else self.controller.default_font["size"]
        self.style.configure(self.style_name, font=self.font, rowheight=int(math.ceil(
            self.font.metrics()['linespace']*(1.125 if str(sys.platform) == "darwin" else 1.3))))
    ...
    def set_font_family(self):
        ...
        # Necessary because turning off 'custom font' option seems to fallback to font_size of '9'.
        self.set_font_size()

    def set_yes_no(self):
        for i in range(0, 2):
            change = self.controller.yes_no.get()

            self.bool_menu.entryconfigure(
                i, label=self.bool_options[change][i])

            self.change_yes_no_recursively(self._tree.get_children()[0])

    def change_yes_no_recursively(self, parent):
        self._tree.item(parent, open=True)
        change = self.controller.yes_no.get()
        for child in self._tree.get_children(parent):
            item = self._tree.item(child)['values']
            if type(item[1]) != str:
                self.change_yes_no_recursively(child)
                continue

            which = 0 if item[1].lower() == "true" or item[1].lower() == "yes" else 1

            if 'boolean' in item[0].lower():
                item[1] = self.bool_options[change][which]
                self._tree.item(child, values=item)
    ...
    def qualify_value(self, value, value_type):
        ...
        if value_type == "data":
            if self.data_type_string.get().lower() == "hex":
                ...
                value = "<{}>".format(
                    " ".join((value[0+i:8+i] for i in range(0, len(value), 8))).upper())
            else:
                value = value.rstrip("=")
            ...
            try:
                value = datetime.datetime.strptime(
                    value, "%b %d, %Y %I:%M:%S %p").strftime("%b %d, %Y %I:%M:%S %p")
            except:
                try:
                    value = datetime.datetime.strptime(
                        value, "%Y-%m-%d %H:%M:%S %z").strftime("%b %d, %Y %I:%M:%S %p")
                except:
            ...
            if self.int_type_string.get().lower() == "hex" and not isinstance(value, float) and value >= 0:
                ...
                if len(value) % 2:
                    value = "0"+value  # Pad to an even number of chars
                value = "0x"+value
            value = str(value)
        elif value_type == "boolean":
            if not value.lower() in ("true", "false", "yes", "no"):
                return (False, "Invalid Boolean Data", "Booleans can only be True/False.")
            value = self.bool_options[self.controller.yes_no.get()][0] if value.lower(
            ) == "true" or value.lower() == "yes" else self.bool_options[self.controller.yes_no.get()][1]
        return (True, value)
    ...
    def do_replace(self, node, find, new_text):
        ...
        if find_type == "key":
            ...
            new_name = re.sub(("" if case_sensitive else "(?i)") +
                              re.escape(find), lambda m: new_text, name)
        ...
        if find_type == "string":
            ...
            values[1] = re.sub(("" if case_sensitive else "(?i)") +
                               re.escape(find), lambda m: new_text, values[1])
            ...
            if self.data_type_string.get().lower() == "hex":
                ...
                new_text = new_text.replace(" ", "").replace(
                    "<", "").replace(">", "").upper()
                values[1] = values[1].upper().replace(" ", "").replace(
                    "<", "").replace(">", "").upper().replace(find, new_text)
                values[1] = "<{}>".format(
                    " ".join((values[1][0+i:8+i] for i in range(0, len(values[1]), 8))))
            else:
    ...
    def replace(self, event=None):
        ...
        if not len(find):
            self.bell()
            mb.showerror(
                "Nothing To Find", "The find textbox is empty, nothing to search for.", parent=self)
            return None
        ...
        if find == repl:
            ...
            self.bell()
            mb.showerror("Find and Replace Are Identical",
                         "The find and replace values are the same.  Nothing to do.", parent=self)
            return
        # Find out if we're replacing everything or not
        replace_all = self.r_all_var.get()
        node = "" if not len(self._tree.selection()
                             ) else self._tree.selection()[0]
        ...
        if replace_all:
            ...
            if not len(matches):
                ...
                self.bell()
                mb.showerror("No Replaceable Matches Found", '"{}" did not match any {} fields in the current plist.'.format(
                    find, self.find_type.lower()), parent=self)
                return
    ...
    def find_prev(self, event=None):
        ...
        if not len(matches):
            # Nothing found - let's throw an error
            self.bell()
            mb.showerror("No Matches Found", '"{}" did not match any {} fields in the current plist.'.format(
                type_check[1], self.find_type.lower()), parent=self)
            return None
    ...
    def start_editing(self, event=None):
        ...
        node = self.get_root_node() if not len(
            self._tree.selection()) else self._tree.selection()[0]
        parent = self._tree.parent(node)
        parent_type = "dictionary" if not len(
            parent) else self.get_check_type(parent).lower()
    ...
    def get_min_max_from_match(self, match_text):
        ...
        temp_min = "0.0.0"
        temp_max = "99.99.99"
        # Strip out "1" in prefix matching to match any
        match_text = "" if match_text == "1" else match_text
        if match_text != "":
            try:
                min_list = match_text.split(".")
                max_list = [x for x in min_list]
                # pad it out with 0s for min
                min_list += ["0"] * (3-len(min_list))
                # Ensure all blanks are 0s too
                min_list = [x if len(x) else "0" for x in min_list]
                # pad it with 99s for max
                max_list += ["99"] * (3-len(max_list))
                # Ensure all blanks are 0s too
                max_list = [x if len(x) else "99" for x in max_list]
    ...
    def oc_snapshot(self, event=None, clean=False):
        target_dir = os.path.dirname(self.current_plist) if self.current_plist and os.path.exists(
            os.path.dirname(self.current_plist)) else None
        oc_folder = fd.askdirectory(
            title="Select OC Folder:", initialdir=target_dir)
        # Lift the window to continue catching events
        ...
        for x in (oc_acpi, oc_drivers, oc_kexts):
            if not os.path.exists(x):
                self.bell()
                mb.showerror("Incorrect OC Folder Struction",
                             "{} does not exist.".format(x), parent=self)
                return
            if x != oc_efi and not os.path.isdir(x):
                self.bell()
                mb.showerror("Incorrect OC Folder Struction",
                             "{} exists, but is not a directory.".format(x), parent=self)
                return
        ...
        if target_snap and target_snap != select_snap:  # Version mismatch - warn
            found_ver = "{} -> {}".format(target_snap.get(
                "min_version", "0.0.0"), target_snap.get("max_version", "Current"))
            select_ver = "{} -> {}".format(select_snap.get(
                "min_version", "0.0.0"), select_snap.get("max_version", "Current"))
        ...
        for path, subdirs, files in os.walk(oc_acpi):
            for name in files:
                if not name.startswith(".") and name.lower().endswith(".aml"):
                    new_acpi.append(os.path.join(path, name)[
                                    len(oc_acpi):].replace("\\", "/").lstrip("/"))
            ...
            for name in sorted(subdirs, key=lambda x: x.lower()):
                ...
                for kpath, ksubdirs, kfiles in os.walk(os.path.join(path, name)):
                    for kname in kfiles:
                        if kname.lower() == "info.plist":
                            ...
                            plist_rel_path = plist_full_path[len(os.path.join(
                ...
                try:
                    ...
                    if info_plist.get("CFBundleExecutable", None):
                        ...
                        kdict["ExecutablePath"] = "Contents/MacOS/" + \
        ...
        original_kexts = [x for x in kexts if x.get(
            "BundlePath", "") in bundle_list]
        ...
        unordered_kexts = []
        for x in new_kexts:
            x = next((y for y in kext_list if y[0].get(
                "BundlePath", "") == x.get("BundlePath", "")), None)
            if not x:
                continue
            parents = [next((z for z in new_kexts if z.get("BundlePath", "") == y[0].get("BundlePath", "")), [
            ]) for y in kext_list if y[1].get("CFBundleIdentifier", None) in x[1].get("OSBundleLibraries", [])]
            children = [next((z for z in new_kexts if z.get("BundlePath", "") == y[0].get("BundlePath", "")), [
            ]) for y in kext_list if x[1].get("CFBundleIdentifier", None) in y[1].get("OSBundleLibraries", [])]
            parents = [y for y in parents if not y in children and not y.get(
                "BundlePath", "") == x[0].get("BundlePath", "")]
        ...
        while len(unordered_kexts):  # This could be dangerous if things aren't properly prepared above
            ...
            if len(kext["parents"]):
                disabled_parents.extend([x.get("BundlePath", "") for x in kext["parents"] if x.get(
                    "Enabled", True) == False and not x.get("BundlePath", "") in disabled_parents])
                if not all(x in ordered_kexts for x in kext["parents"]):
                    unordered_kexts.append(kext)
                    continue
            ordered_kexts.append(next(x for x in new_kexts if x.get(
                "BundlePath", "") == kext["kext"].get("BundlePath", "")))
        ...
        rearranged = []
        while True:
            check1 = [x.get("BundlePath", "") for x in ordered_kexts if not x.get(
                "BundlePath", "") in rearranged]
            check2 = [x.get("BundlePath", "") for x in original_kexts if not x.get(
                "BundlePath", "") in rearranged]
            out_of_place = next(
                (x for x in range(len(check1)) if check1[x] != check2[x]), None)
            if out_of_place == None:
                break
        ...
        for kext in ordered_kexts:
            ...
            info = next((x for x in kext_list if x[0].get(
                "BundlePath", "") == temp_kext.get("BundlePath", "")), None)
            ...
            comp_kexts = [x for x in enabled_kexts if x[1]
                          ["CFBundleIdentifier"] == info[1]["CFBundleIdentifier"]]
                ...
                for name in files:
                    if not name.startswith(".") and name.lower().endswith(".efi"):
                        ...
                        new_tool_entry = {
                            ...
                            "Path": os.path.join(path, name)[len(oc_tools):].replace("\\", "/").lstrip("/")
                        }
                        # Add our snapshot custom entries, if any
                        for x in tool_add:
            ...
            for path, subdirs, files in os.walk(oc_drivers):
                for name in files:
                    if not name.startswith(".") and name.lower().endswith(".efi"):
                        ...
                        if not driver_add:
                            drivers_list.append(os.path.join(path, name)[len(oc_drivers):].replace(
            ...
            if t == "data":
                ...
                if new_display.lower() == "hex":
                    ...
                    if sys.version_info < (3, 0):
                        ...
                        value = binascii.hexlify(base64.b64decode(
                    ...
                    # Assume base64
                    if sys.version_info < (3, 0):
                        value = base64.b64encode(binascii.unhexlify(
                            value.replace("<", "").replace(">", "").replace(" ", "")))
                    else:
                        value = base64.b64encode(binascii.unhexlify(value.replace("<", "").replace(
    ...
    def move_selection(self, event):
        ...
        # Force Root node as needed
        tv_item = self.get_root_node() if tv_item == "" else tv_item
        ...
        # we'll add as a child, uper half will add as a sibling
            ...
            if y+(height/2) <= event.y < y+height and self._tree.parent(tv_item) != "":
                ...
                move_to = 0
        target = self.get_root_node() if not len(
            self._tree.selection()) else self._tree.selection()[0]
        ...
        if not self.drag_undo:
            self.drag_undo = {"from": self._tree.parent(target), "index": self._tree.index(
                target), "name": self._tree.item(target, "text")}
        try:
    ...
    def confirm_drag(self, event):
        ...
        if verify:
            names = [self._tree.item(x, "text") for x in self._tree.get_children(
                node) if not x == target]
    ...
    def strip_disabled(self, event=None):
        ...
        for node in nodes:
            ...
            value = values[1]
            check_type = self.get_check_type(node).lower()
            if check_type == "boolean" and (name == "enabled" and value == self.bool_options[self.controller.yes_no.get()][1]) or (name == "disabled" and value == self.bool_options[self.controller.yes_no.get()][0]):
    ...
    def get_title(self):
        name = self.title()[0:-len(" - Edited")
                            ] if self.edited else self.title()
    ...
    def check_save(self):
        ...
        answer = mb.askyesnocancel(
            "Unsaved Changes", "Save changes to {}?".format(self.get_title()))
        ...
        try:
            if self.plist_type_string.get().lower() == "binary":
                with open(temp_file, "wb") as f:
                    plist.dump(plist_data, f, sort_keys=self.controller.settings.get(
                        "sort_dict", False), fmt=plist.FMT_BINARY)
            # elif not self.xcode_data:
                ...
                plist_text = plist.dumps(plist_data, sort_keys=self.controller.settings.get(
                    "sort_dict", False)).split("\n")
                new_plist = []
                data_tag = ""
                types = ("</array>", "</dict>")
                for i, x in enumerate(plist_text):
                    x_stripped = x.strip()
                    try:
                        type_check = types[types.index(
                            x_stripped)].replace("</", "<")
                        if plist_text[i-1].strip() == type_check:
                            new_plist[-1] = x.replace("</",
                                                      "<").replace(">", "/>")
                            data_tag = ""
                            continue
            ...
            # Had an issue, throw up a display box
            self.bell()
            mb.showerror("An Error Occurred While Saving",
                         repr(e), parent=self)
            return None
        finally:
            try:
    ...
    def copy_selection(self, event=None):
        ...
        if node == "":
            ...
            return
        try:
            clipboard_string = plist.dumps(self.nodes_to_values(
                node, None), sort_keys=self.controller.settings.get("sort_dict", False))
            # Get just the values
            self.clipboard_clear()
    ...
    def copy_children(self, event=None):
        ...
        try:
            ...
            clipboard_string = plist.dumps(
                plist_data, sort_keys=self.controller.settings.get("sort_dict", False))
            self.clipboard_clear()
            self.clipboard_append(clipboard_string)
        except:
            pass
    ...
    def copy_all(self, event=None):
        try:
            clipboard_string = plist.dumps(self.nodes_to_values(self.get_root_node(
            ), None), sort_keys=self.controller.settings.get("sort_dict", False))
            # Get just the values
            self.clipboard_clear()
            self.clipboard_append(clipboard_string)
        except:
    ...
    def paste_selection(self, event=None):
        ...
        plist_data = None
        try:
            plist_data = plist.loads(clip, dict_type=dict if self.controller.settings.get(
                "sort_dict", False) else OrderedDict)
        except:
            # May need the header
            ...
            element_type = "dict" if clip_check.startswith("<key>") else "array" if not clip_check.startswith(
                "<array>") and len(clip_check.split("\n")) > 1 else None
            if element_type:
                cb_list.insert(1, "<{}>".format(element_type))
                cb_list.insert(3, "</{}>".format(element_type))
            cb = "\n".join(cb_list)
            try:
                plist_data = plist.loads(cb, dict_type=dict if self.controller.settings.get(
                    "sort_dict", False) else OrderedDict)
            except Exception as e:
                # Let's throw an error
                self.bell()
                mb.showerror("An Error Occurred While Pasting",
                             repr(e), parent=self)
                return 'break'
        if plist_data == None:
            if len(clip):
                ...
                self.bell()
                mb.showerror("An Error Occurred While Pasting",
                             "The pasted value is not a valid plist string.", parent=self)
            # Nothing to paste
            return 'break'
        node = "" if not len(self._tree.selection()
                             ) else self._tree.selection()[0]
        ...
        if isinstance(plist_data, list):  # Convert to a dict to add
            new_plist = {} if self.controller.settings.get(
                "sort_dict", False) else OrderedDict()
        ...
        if not isinstance(plist_data, dict):
            ...
            if node == self.get_root_node() and self.get_root_type() == None:
                ...
                add_list.append({"type": "edit", "cell": node, "text": self._tree.item(
                    node, "text"), "values": self._tree.item(node, "values")})
                if self.is_data(plist_data):
                    self._tree.item(node, values=(self.get_type(
                        plist_data), self.get_data(plist_data), "",))
                elif isinstance(plist_data, datetime.datetime):
                    self._tree.item(node, values=(self.get_type(
                        plist_data), plist_data.strftime("%b %d, %Y %I:%M:%S %p"), "",))
                else:
                    self._tree.item(node, values=(
                        self.get_type(plist_data), plist_data, "",))
            else:
                # I guess we're not - let's force it into a dict to be savage
                plist_data = {"New item": plist_data}
        if isinstance(plist_data, dict):
            dict_list = list(plist_data.items()) if not self.controller.settings.get(
                "sort_dict", False) else sorted(list(plist_data.items()))
            names = [self._tree.item(x, "text") for x in self._tree.get_children(
                node)] if t == "dictionary" else []
        ...
        first = self.get_root_node() if not len(
            add_list) else add_list[0].get("cell")
    ...
    def add_node(self, value, parentNode="", key=None):
        ...
        if isinstance(value, (list, tuple)):
            children = "1 child" if len(
                value) == 1 else "{} children".format(len(value))
            values = (self.get_type(value), children,
                      "" if parentNode == "" else self.drag_code)
        elif isinstance(value, dict):
            children = "1 key/value pair" if len(
                value) == 1 else "{} key/value pairs".format(len(value))
            values = (self.get_type(value), children,
                      "" if parentNode == "" else self.drag_code)
        else:
            values = (self.get_type(value), value,
                      "" if parentNode == "" else self.drag_code)
        i = self._tree.insert(parentNode, "end", text=key, values=values)

        if isinstance(value, dict):
            self._tree.item(i, open=True)
            dict_list = list(value.items()) if not self.controller.settings.get(
                "sort_dict", False) else sorted(list(value.items()))
            for (key, val) in dict_list:
            ...
            self._tree.item(i, values=(self.get_type(value), self.get_data(
                value), "" if parentNode == "" else self.drag_code,))
        elif isinstance(value, datetime.datetime):
            self._tree.item(i, values=(self.get_type(value), value.strftime(
                "%b %d, %Y %I:%M:%S %p"), "" if parentNode == "" else self.drag_code,))
            ...
            try:
                ...
                value = "0x00"  # Default to 0
            self._tree.item(i, values=(
                v_type, value, "" if parentNode == "" else self.drag_code,))
        else:
            self._tree.item(i, values=(self.get_type(value), value,
                            "" if parentNode == "" else self.drag_code,))
        return i
    ...
    def get_value_from_node(self, node=""):
        ...
        if check_type == "dictionary":
            value = {} if self.controller.settings.get(
                "sort_dict", False) else OrderedDict()
        elif check_type == "array":
            value = []
        elif check_type == "boolean":
            value = True if values[1].lower(
            ) == "true" or values[1].lower() == "yes" else False
            ...
            if self.data_type_string.get().lower() == "hex":
                # Convert the hex
                if sys.version_info < (3, 0):
                    value = plistlib.Data(binascii.unhexlify(
                        value.replace("<", "").replace(">", "").replace(" ", "")))
                else:
                    value = binascii.unhexlify(value.replace("<", "").replace(
    ...
    def get_unique_name(self, name, names, int_check=False):
        ...
        while True:
            temp_name = str(name+num) if isinstance(name,
                                                    int) else name if num == start else name+str(sep)+str(num)
    ...
    def new_row(self, target=None, force_sibling=False):
        if target == None or isinstance(target, tk.Event):
            target = "" if not len(self._tree.selection()
                                   ) else self._tree.selection()[0]
        ...
        # Force the Root node if need be
        target = self.get_root_node() if target == "" else target
        # create a unique name
        name = ""
        if self.get_check_type(target).lower() == "dictionary":
            names = [self._tree.item(x, "text")
                     for x in self._tree.get_children(target)]
            name = self.get_unique_name("New String", names)
        new_cell = self._tree.insert(target, "end", text=name, values=(
            self.menu_code + " String", "", self.drag_code,))
    ...
    def remove_row(self, target=None):
        ...
        new_target = parent if not len(
            remaining) else remaining[target_index] if target_index < len(remaining) else remaining[-1]
    ...
    def update_children(self, target):
        ...
        if self.get_check_type(target).lower() == "dictionary":
            children = "1 key/value pair" if len(
                child_count) == 1 else "{} key/value pairs".format(len(child_count))
        elif self.get_check_type(target).lower() == "array":
            children = "1 child" if len(
                child_count) == 1 else "{} children".format(len(child_count))
    ...
    def change_type(self, value, cell=None):
        # Need to walk the values and pad
        if cell == None:
            cell = "" if not len(self._tree.selection()
                                 ) else self._tree.selection()[0]
        ...
        if value.lower() == "number":
            values[1] = "0" if self.int_type_string.get(
            ).lower() == "decimal" else "0x00"
        elif value.lower() == "boolean":
            values[1] = self.bool_options[self.controller.yes_no.get()][0]
            ...
            values[1] = datetime.datetime.now().strftime(
                "%b %d, %Y %I:%M:%S %p")
        elif value.lower() == "data":
            values[1] = "<>" if self.data_type_string.get(
            ).lower() == "hex" else ""
        else:
            values[1] = ""
        # Set the values
    ...
    def set_bool(self, value):
        # Need to walk the values and pad
        values = self.get_padded_values("" if not len(
            self._tree.selection()) else self._tree.selection()[0], 3)
        cell = "" if not len(self._tree.selection()
                             ) else self._tree.selection()[0]
        self.add_undo({
            "type": "edit",
        ...
        i = 0 if value.lower() == "true" else 1
        values[1] = self.bool_options[self.controller.yes_no.get()][i]

        # Set the values
        self._tree.item("" if not len(self._tree.selection())
                        else self._tree.selection()[0], values=values)
        if not self.edited:
            self.edited = True
            self.title(self.title()+" - Edited")
    ...
    def get_cell_path(self, cell=None):
        ...
        while True:
            ...
            if not self.get_check_type(cell).lower() == "array":
                ...
                path.append(self._tree.item(
                    current_cell, "text").replace("/", "\/"))
            else:
                path.append("*")
            current_cell = cell
        return "/".join(path[::-1])
    ...
    def merge_menu_preset(self, val=None):
        ...
        if not len(paths) == len(types):
            self.bell()
            mb.showerror("Incorrect Patch Format",
                         "Patch is incomplete.", parent=self)
            return
        if not len(paths) or paths[0] == "*":
            # No path, or it's an array
            self.bell()
            mb.showerror("Incorrect Patch Format",
                         "Patch starts with an array - must be a dictionary.", parent=self)
            return
        ...
        for p, t in izip(paths, types):
            found = False
            needed_type = {"d": "Dictionary", "a": "Array"}.get(
                t.lower(), "Dictionary")
            ...
            if not found:
                # Need to add it
                current_cell = self._tree.insert(current_cell, "end", text=p, values=(
                    self.menu_code+" "+needed_type, "", self.drag_code,), open=True)
                undo_list.append({
                    "type": "add",
                    "cell": current_cell
                })
    ...
    def do_sort(self, cell, recursive=False, reverse=False):
        ...
        if not len(children):
            return undo_tasks  # bail early, nothing to check
        sorted_children = [x for x in children]
        if self.get_check_type(cell).lower() == "dictionary":
            sorted_children = sorted(sorted_children, key=lambda x: self._tree.item(
                x, "text").lower(), reverse=reverse)
        skip_sort = all((children[x] == sorted_children[x]
                        for x in range(len(children))))
        for index, child in enumerate(sorted_children):
            if self.get_check_type(child).lower() in ("dictionary", "array") and recursive:
                undo_tasks.extend(self.do_sort(
                    child, recursive=recursive, reverse=reverse))
            if skip_sort:
                continue  # No change - skip sorting
            # Add the move command
            undo_tasks.append({
                "type": "move",
                "cell": child,
    ...
    def popup(self, event):
        ...
        if self.get_check_type(cell).lower() in ("array", "dictionary"):
            popup_menu.add_command(label="Expand Node",
                                   command=self.expand_node)
            popup_menu.add_command(label="Collapse Node",
                                   command=self.collapse_node)
            popup_menu.add_separator()
            popup_menu.add_command(
                label="Expand Children", command=self.expand_children)
            popup_menu.add_command(
                label="Collapse Children", command=self.collapse_children)
            popup_menu.add_separator()
        ...
        if cell in ("", self.get_root_node()):
            ...
            if self.get_check_type(self.get_root_node()).lower() in ("array", "dictionary"):
                popup_menu.add_command(label="New top level entry{}".format(
                    " (+)" if is_mac else ""), command=lambda: self.new_row(self.get_root_node()), accelerator=None if is_mac else "(+)")
        else:
            if self.get_check_type(cell).lower() in ("dictionary", "array") and (self._tree.item(cell, "open") or not len(self._tree.get_children(cell))):
                popup_menu.add_command(label="New child under '{}'{}".format(self._tree.item(
                    cell, "text"), " (+)" if is_mac else ""), command=lambda: self.new_row(cell), accelerator=None if is_mac else "(+)")
                popup_menu.add_command(label="New sibling of '{}'".format(
                    self._tree.item(cell, "text")), command=lambda: self.new_row(cell, True))
                popup_menu.add_command(label="Remove '{}' and any children{}".format(self._tree.item(
                    cell, "text"), " (-)" if is_mac else ""), command=lambda: self.remove_row(cell), accelerator=None if is_mac else "(-)")
            else:
                popup_menu.add_command(label="New sibling of '{}'{}".format(self._tree.item(
                    cell, "text"), " (+)" if is_mac else ""), command=lambda: self.new_row(cell), accelerator=None if is_mac else "(+)")
                popup_menu.add_command(label="Remove '{}'{}".format(self._tree.item(
                    cell, "text"), " (-)" if is_mac else ""), command=lambda: self.remove_row(cell), accelerator=None if is_mac else "(-)")
        # Let's get our sorting menus
        parent = cell if cell in (
            "", self.get_root_node()) else self._tree.parent(cell)
        if self.get_check_type(parent).lower() in ("dictionary", "array"):
            ...
            recurs_target = cell if self.get_check_type(cell).lower() in (
                "dictionary", "array") and len(self._tree.get_children(cell)) else parent
            popup_menu.add_command(label="Recursively sort keys starting at '{}'".format(self._tree.item(
                recurs_target, "text")), command=lambda: self.sort_keys(recurs_target, recursive=True))
            popup_menu.add_command(label="Recursively reverse sort keys starting at '{}'".format(self._tree.item(
                recurs_target, "text")), command=lambda: self.sort_keys(recurs_target, recursive=True, reverse=True))
            # Check the actual cell
            sort_target = cell if cell in ("", self.get_root_node()) or (self.get_check_type(cell).lower(
            ) == "dictionary" and len(self._tree.get_children(cell)) > 1) else self._tree.parent(cell)
            popup_menu.add_command(label="Sort keys in '{}'".format(self._tree.item(
                sort_target, "text")), command=lambda: self.sort_keys(sort_target))
            popup_menu.add_command(label="Reverse sort keys in '{}'".format(self._tree.item(
                sort_target, "text")), command=lambda: self.sort_keys(sort_target, reverse=True))

        # Add the copy and paste options
        popup_menu.add_separator()
        c_state = "normal" if len(self._tree.selection()) else "disabled"
        try:
            p_state = "normal" if len(
                self.root.clipboard_get()) else "disabled"
        except:
            p_state = "disabled"  # Invalid clipboard content
        popup_menu.add_command(label="Copy{}".format(" (Cmd+C)" if is_mac else ""),
                               command=self.copy_selection, state=c_state, accelerator=None if is_mac else "(Ctrl+C)")
        if not cell in ("", self.get_root_node()) and self.get_check_type(cell).lower() in ("array", "dictionary"):
            popup_menu.add_command(label="Copy Children",
                                   command=self.copy_children, state=c_state)
        popup_menu.add_command(label="Paste{}".format(" (Cmd+V)" if is_mac else ""),
                               command=self.paste_selection, state=p_state, accelerator=None if is_mac else "(Ctrl+V)")
            ...
            for item in sorted(valid):
                ...
                for x in options[item]:
                    if x.get("separator", False) != False:
                        ...
                        item_menu.add("command", label=x.get(
                        ...
                        item_menu.add_command(
                            label=name, command=lambda item=passed: self.merge_menu_preset(item))
                parts = self.split(item)
                option_menu.add_cascade(
                    label=" -> ".join(parts[1 if parts[0].lower() == "root" and len(parts) > 1 else 0:]), menu=item_menu)
    ...
    def on_double_click(self, event):
        ...
        if index == 1:
            ...
            return 'break'
        if parent == "" and (index == 0 or self.get_check_type(self.get_root_node()).lower() in ("array", "dictionary")):
        ...
        cell = self._tree.item("" if not len(
            self._tree.selection()) else self._tree.selection()[0])
    ...
    def set_colors(self, event=None):
        ...
        self.r1t = self.controller.text_color(
            self.r1, invert=self.controller.r1_inv_check.get())
        self.r2t = self.controller.text_color(
            self.r2, invert=self.controller.r2_inv_check.get())
        self.hlt = self.controller.text_color(
            self.hl, invert=self.controller.hl_inv_check.get())
        self.style.configure(
            self.style_name, background=self.bg, fieldbackground=self.bg)
        self.style.map(self.style_name, background=[
                       ("selected", self.hl)], foreground=[("selected", self.hlt)])
        self._tree.tag_configure(
            'even', foreground=self.r1t, background=self.r1)
        self._tree.tag_configure(
            'odd', foreground=self.r2t, background=self.r2)
        self._tree.tag_configure(
            "selected", foreground="black", background=self.hl)
        self.alternate_colors()
