            ...
            for child in self.parent.get_children(parent):
                ...
                if text == self.parent.item(child, "text"):
                    ...
                    if not event is None:
                        self.bell()
                        if not mb.askyesno("Invalid Key Name",
...
"""
    ...
    def replace(self, event=None):
        ...
        if replace_all:
            ...
            if not len(matches):
                ...
                self.bell()
                mb.showerror("No Replaceable Matches Found",
    ...
    def is_match(self, node, text):
        ...
        if node_type == "data":
            # if hex, we need to strip spaces and brackets, upper() both, and compare
            if self.data_type_string.get().lower() == "hex":
                if text.replace(" ", "").replace("<", "").replace(">", "").upper() in value.replace(" ", "").replace(
                        "<", "").replace(">", "").upper():
                    # Got a match!
                    return True
            else:
    ...
    def reload_from_disk(self, event=None):
        ...
        if self.edited:
            ...
            if not mb.askyesno("Unsaved Changes",
                               "Any unsaved changes will be lost when reloading from disk. Continue?", parent=self):
                return
    ...
    def oc_snapshot(self, event=None, clean=False):
        ...
        for snap in self.controller.snapshot_data:
            ...
            if user_snap.lower() != "latest" and user_snap >= snap.get("min_version", "0.0.0") and snap.get(
                    "min_version", "0.0.0") > select_snap.get("min_version", "0.0.0"):
                select_snap = snap
        ...
        if target_snap and target_snap != select_snap:  # Version mismatch - warn
            ...
            if mb.askyesno("Snapshot Version Mismatch",
        ...
        if len(rearranged):
            if not mb.askyesno("Incorrect Kext Load Order",
        ...
        if len(disabled_parents):
            if mb.askyesno("Disabled Parent Kexts",
        ...
        if len(duplicate_bundles):
            if mb.askyesno("Duplicate CFBundleIdentifiers",
        ...
        new_efi = [x for x in os.listdir(oc_drivers) if x.lower().endswith(
            ".efi") and not x.startswith(".")]
        ...
        if self.controller.settings.get("force_snapshot_schema", False):
            ...
            for entries, values in ((tree_dict["ACPI"]["Add"], acpi_add), (tree_dict["Kernel"]["Add"], kext_add),
    ...
    def strip_disabled(self, event=None):
        ...
        for node in nodes:
            ...
            value = values[1]
            check_type = self.get_check_type(node).lower()
            if check_type == "boolean" and (name == "enabled" and value == "False") or (
                    name == "disabled" and value == "True"):
    ...
    def add_node(self, value, parentNode="", key=None):
        ...
        elif isinstance(value, (int, long)) and not isinstance(value,
                                                               bool) and self.int_type_string.get().lower() == "hex":
    ...
    def new_row(self, target=None, force_sibling=False):
        ...
        new_cell = None
        if not self.get_check_type(target).lower() in ("dictionary", "array") or force_sibling or (
                not self._tree.item(target, "open") and len(self._tree.get_children(target))):
            ...
            for x in self._tree.get_children(current_cell):
                ...
                if cell_name == p:
                    ...
                    if not current_type.lower() == needed_type.lower():
                        ...
                        self.bell()
                        if mb.askyesno("Incorrect Type",
                                       "{} is {}, should be {}.\n\nWould you like to replace it?".format(cell_name,
                                                                                                         current_type,
                                                                                                         needed_type),
                                       parent=self):
            ...
            for x in self._tree.get_children(current_cell):
                ...
                if name in value:
                    if not replace_asked:
                        # Ask first
                        self.bell()
                        if mb.askyesno("Key(s) Already Exist",
        ...
        if cell in ("", self.get_root_node()):
            ...
            if self.get_check_type(self.get_root_node()).lower() in ("array", "dictionary"):
                popup_menu.add_command(label="New top level entry{}".format(
                    " (+)" if is_mac else ""), command=lambda: self.new_row(self.get_root_node()),
                    accelerator=None if is_mac else "(+)")
        else:
            if self.get_check_type(cell).lower() in ("dictionary", "array") and (
                ...
                popup_menu.add_command(label="New child under '{}'{}".format(self._tree.item(
                    cell, "text"), " (+)" if is_mac else ""), command=lambda: self.new_row(cell),
                    accelerator=None if is_mac else "(+)")
                ...
                popup_menu.add_command(label="Remove '{}' and any children{}".format(self._tree.item(
                    cell, "text"), " (-)" if is_mac else ""), command=lambda: self.remove_row(cell),
                    accelerator=None if is_mac else "(-)")
            else:
